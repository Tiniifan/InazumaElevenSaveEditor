...
namespace InazumaElevenSaveEditor.Formats.Player_Files
{
    public class INZ6 : IPlayerFiles
    {
        public string Extension => ".inz6";

        public string GameNameCode => "IEGOGALAXY";

        private IDictionary<UInt32, Player> Players => Common.InazumaElevenGo.Players.Galaxy;

        private IDictionary<UInt32, Move> Moves => Common.InazumaElevenGo.Moves.Galaxy;

        private IDictionary<UInt32, Avatar> Avatars => Common.InazumaElevenGo.Avatars.Galaxy;

        private IDictionary<UInt32, Equipment> Equipments => Common.InazumaElevenGo.Equipments.Galaxy;
        ...
        private string PlayerToString(Player player)
        {
            string exportContent = string.Join("\n", new String[]
            {
                ...
                player.Moves[0].Name + "|" + player.Moves[0].Level.ToString() + "|" + player.Moves[0].TimeLevel + "|" + player.Moves[0].Unlock,
                player.Moves[1].Name + "|" + player.Moves[1].Level.ToString() + "|" + player.Moves[1].TimeLevel + "|" + player.Moves[1].Unlock,
                player.Moves[2].Name + "|" + player.Moves[2].Level.ToString() + "|" + player.Moves[2].TimeLevel + "|" + player.Moves[2].Unlock,
                player.Moves[3].Name + "|" + player.Moves[3].Level.ToString() + "|" + player.Moves[3].TimeLevel + "|" + player.Moves[3].Unlock,
                player.Moves[4].Name + "|" + player.Moves[4].Level.ToString() + "|" + player.Moves[4].TimeLevel + "|" + player.Moves[4].Unlock,
                player.Moves[5].Name + "|" + player.Moves[5].Level.ToString() + "|" + player.Moves[5].TimeLevel + "|" + player.Moves[5].Unlock,
        ...
        private Player StringToPlayer(List<string> playerString, bool keepInvestedPoint)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                if (move != null)
                {
                    ...
                    move.TimeLevel = Convert.ToInt32(moveValues[2]);
                ...
                else
                {
                    if (i < 4)
                    {
                        ...
                        move.TimeLevel = move.EvolutionSpeed.TimeLevel[0];
                    }
                    else
                    {
                        move = Moves[0x0];

...
namespace InazumaElevenSaveEditor.InazumaEleven.Games.IEGO
{
    public class GOJp : IGame
    {
        ...
        {
            // Group Item 1
            Data.Seek((uint)IEGOJpHelper.ItemBlockGroup1Offset);
            Dictionary<int, Item> itemBlockGroup1 = Data.ReadMultipleStruct<IEGOJpHelper.ItemBlockGroup1>(256).Where(x => Items.ContainsKey(x.ID) && x.ID != 0x00).ToDictionary(x => x.Index, y => new Item(y.ID, Items[y.ID], y.Quantity));

            // Group Item 2
            Data.Seek((uint)IEGOJpHelper.ItemBlockGroup2Offset);
            Dictionary<int, Item> itemBlockGroup2 = Data.ReadMultipleStruct<IEGOJpHelper.ItemBlockGroup2>(224).Where(x => Items.ContainsKey(x.ID) && x.ID != 0x00).ToDictionary(x => x.Index, y => new Item(y.ID, Items[y.ID], y.Quantity, y.QuantityEquiped));

            // Group Item 3
            Data.Seek((uint)IEGOJpHelper.ItemBlockGroup3Offset);
            Dictionary<int, Item> itemBlockGroup3 = Data.ReadMultipleStruct<IEGOJpHelper.ItemBlockGroup3>(512).Where(x => Items.ContainsKey(x.ID) && x.ID != 0x00).ToDictionary(x => x.Index, y => new Item(y.ID, Items[y.ID]));
        ...
        {
            // Group Item 1
            writer.Seek((uint)IEGOJpHelper.ItemBlockGroup1Offset);
            ...
            // Group Item 2
            writer.Seek((uint)IEGOJpHelper.ItemBlockGroup2Offset);
            ...
            // Group Item 3
            writer.Seek((uint)IEGOJpHelper.ItemBlockGroup3Offset);
        ...
        {
            // Get player index
            Data.Seek((uint)IEGOJpHelper.PlayerIndexOffset);
            int[] index = Data.ReadMultipleStruct<int>(IEGOJpHelper.MaximumPlayer)
                .Where(x => x != 0x0)
                .ToArray();

            // Get player data according to their index
            Data.Seek((uint)IEGOJpHelper.PlayerDataOffset);
            List<IEGOJpHelper.PlayerBlock> playerBlocks = Data.ReadMultipleStruct<IEGOJpHelper.PlayerBlock>(index.Length)
                .ToList();
        ...
        {
            // Save player index
            writer.Seek((uint)IEGOJpHelper.PlayerIndexOffset);
            for (int i = 0; i < IEGOJpHelper.MaximumPlayer; i++)
            {
                if (i < Reserve.Count)
                {
            ...
            // Save player data
            writer.Seek((uint)IEGOJpHelper.PlayerDataOffset);
        ...
        public void OpenSaveInfo()
        {
            if (SaveInfo == null)
            {
                ...
                // Name
                Data.Seek((uint)IEGOJpHelper.NameOffset);
                SaveInfo.Name = Data.ReadString(Encoding.UTF8);
                Data.Seek((uint)IEGOJpHelper.TeamNameOffset);
                SaveInfo.TeamName = Data.ReadString(Encoding.UTF8);

                // Time Hours
                Data.Seek((uint)IEGOJpHelper.TimeOffset);
                ...
                // Money
                Data.Seek((uint)IEGOJpHelper.MoneyOffset);
        ...
        private void SaveSaveInfo(BinaryDataWriter writer)
        {
            ...
            {
                writer.Seek((uint)IEGOJpHelper.LinkOffset);
                writer.Write((short)0x01C0);
                writer.Write((short)0x00);
            }

            // Money
            writer.Seek((uint)IEGOJpHelper.MoneyOffset);
        ...
        private Team LoadTeam(uint teamInfo, string teamName, uint teamPlayers)
        {
            ...
            IEGOJpHelper.Team team = Data.ReadStruct<IEGOJpHelper.Team>();
            ...
            IEGOJpHelper.TeamPlayer teamPlayer = Data.ReadStruct<IEGOJpHelper.TeamPlayer>();
